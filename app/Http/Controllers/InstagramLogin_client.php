...
class InstagramLogin_client extends Controller
{
    public function login(Request $request)
{
    ...
    if ($ext->status == 'ok') {
        ...
        // Formatkan string sesuai dengan kebutuhan
        ...
        $cookieClient = \App\Models\Cookie_Client::where('user_id', auth()->id())->first();
        if ($cookieClient === null) {
            \App\Models\Cookie_Client::create([
                'username'   => $uname,
                'pk'         => $uid,
                'cookie_data' => $cookie,
                'useragent'  => $useragent,
            ...
            \App\Models\Cookie_Client::where('user_id', auth()->id())
                ->update([
                    'username'   => $uname,
                    'pk'         => $uid,
                    'cookie_data' => $cookie,
